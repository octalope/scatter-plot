...
var mapValue = (datum, dataRect, plotRect) => {
  var p = {
...
function createChart(options) {
  ...
  var plotRect = {
    x0: yBoxMax.width,
    y0: 0,
    x1: yBoxMax.width + (options.width - yBoxMax.width),
    y1: options.height - xBoxMax.height
  };
  ...
  xAxis
    .rect(options.width - yBoxMax.width, xBoxMax.height)
    .stroke({
      color: 'none',
      opacity: 0.25,
      width: 0.5
    ...
    .fill('#DDD')
    .move(yBoxMax.width, options.height - xBoxMax.height);

  var yAxis = chart.group();
  yAxis
    .rect(yBoxMax.width, options.height - xBoxMax.height)
    .stroke({
      color: 'none',
      opacity: 0.25,
      width: 0.5
  ...
  plotArea
    .rect(options.width - yBoxMax.width, options.height - xBoxMax.height)
    .stroke({
      color: 'none',
      opacity: 0.25,
      width: 0.5
    ...
    .fill('#88F')
    .move(yBoxMax.width, 0);

  options.data.forEach(datum => {
    var point = mapValue(datum, dataRect, plotRect);
    // console.log(JSON.stringify({
    //   datum: datum,
    //   point: point
    // }));
    plotArea.circle(10).move(point.x, point.y);
  });
